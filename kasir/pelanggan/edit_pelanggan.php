...
// Pastikan parameter id dikirimkan via GET
...
$stmt = $mysqli->prepare("SELECT NamaPelanggan, Alamat, NomorTelepon, jk FROM pelanggan WHERE PelangganID = ?");
if (!$stmt) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    ...
    // Jika tidak ada error validasi, update data pelanggan
    if ($message == "") {
        $stmtUpdate = $mysqli->prepare("UPDATE pelanggan SET NamaPelanggan = ?, Alamat = ?, NomorTelepon = ?, jk = ? WHERE PelangganID = ?");
        if ($stmtUpdate) {
            $stmtUpdate->bind_param("ssssi", $namaPelanggan, $alamat, $nomorTelepon, $jk, $pelangganID);
            if ($stmtUpdate->execute()) {
                $message = "Pelanggan berhasil diperbarui!";
                $isSuccess = true;
            } else {
                $message = "Error: Gagal memperbarui data pelanggan. " . $stmtUpdate->error;
            }
            $stmtUpdate->close();
        } else {
...
<head>
    ...
    /* Sama seperti tambah produk: lebar 100% dan padding 20px */
    .content-card {
        width: 100%;
        padding: 20px;
        ...
        <form action="" method="post">
            <div class="form-group">
                ...
                <input type="text" name="namaPelanggan" id="namaPelanggan" class="form-control"
                    placeholder="Masukkan nama pelanggan" value="<?= htmlspecialchars($pelanggan['NamaPelanggan']); ?>"
                    required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nomorTelepon" id="nomorTelepon" class="form-control"
                    placeholder="Masukkan nomor telepon" value="<?= htmlspecialchars($pelanggan['NomorTelepon']); ?>"
                    required>
            </div>
            <div class="form-group">
                ...
                <select name="jk" id="jk" class="form-control" required>
                    ...
                    <option value="Laki_Laki" <?= ($pelanggan['jk'] == 'Laki_Laki') ? 'selected' : ''; ?>>Laki-Laki
                    </option>
                    <option value="Perempuan" <?= ($pelanggan['jk'] == 'Perempuan') ? 'selected' : ''; ?>>Perempuan
                    </option>
                </select>
            </div>
            <button type="submit" class="btn-primary" name="submit">Perbarui Pelanggan</button>
        </form>
